...
class Perceptron:
	...
	def teste(self, amostra):
		...
		if saida_gerada == 1:
			print('Classe: %d. Cliente Confiável (Aprovado)' % saida_gerada)
		else:
			print('Classe: %d. Cliente de Risco (Recusado)' % saida_gerada)
...
#abre arquivo
df = pd.read_csv("amostrasCredito.csv") #tabela dataframe df
arquivo = df.values.tolist() #transforma em lista de lista

amostras = [linha[0:2] for linha in arquivo] #umidade & pressao
for linha in arquivo: # saidas
    saidas = saidas + (linha[2:])
